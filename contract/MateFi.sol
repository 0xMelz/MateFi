...
contract MateFi {
  ...
  event LoanUnderwritten(uint256 id, address lender, uint256 amount);
  ...
  function createLoan(
    ...
    // Create loan
    fraxLoans[loanId].tokenAddress = _tokenAddress;
    fraxLoans[loanId].tokenOwner = msg.sender;
    fraxLoans[loanId].tokenId = _tokenId;
    fraxLoans[loanId].interestRate = _interestRate;
    fraxLoans[loanId].maxLoanAmount = _maxLoanAmount;
    fraxLoans[loanId].loanCompleteTime = _loanCompleteTime;
  ...
  function underwriteLoan(uint256 _loanId, uint256 amount) external {
    ...
    if (loan.firstBidTime != 0) {
      ...
      require(_bidPayout < amount, "Can't underwrite < top lender.");
      // Prevent underwriting a loan with value greater than max bid + pending interest
      require(loan.maxLoanAmount + _totalInterest >= amount, "Can't underwrite > max loan.");
      IERC20(erc20token).transferFrom(msg.sender, address(this), amount);
      // Buyout current top bidder
      bool sent = IERC20(erc20token).transfer(loan.lender, _bidPayout);
      ...
      loan.loanAmount = amount - _totalInterest;
    } else {
      // Prevent underwriting a loan with value greater than max bid
      require(loan.maxLoanAmount >= amount, "Can't underwrite > max loan.");
      // IERC20(erc20token).approve(address(this), amount);
      IERC20(erc20token).transferFrom(msg.sender, address(this), amount);
    ...
    emit LoanUnderwritten(_loanId, msg.sender, amount);
  }
  ...
  function drawLoan(uint256 _loanId) external {
    ...
    bool sent = IERC20(erc20token).transfer(msg.sender, _availableCapital);
  ...
  function repayLoan(uint256 _loanId) external {
    ...
    require(IERC20(erc20token).balanceOf(msg.sender) >= _additionalCapital, "Insufficient repayment.");
    ...
    bool sent = IERC20(erc20token).transferFrom(msg.sender, loan.lender, loan.loanAmount + _totalInterest);
